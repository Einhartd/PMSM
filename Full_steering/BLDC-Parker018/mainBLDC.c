...
void main(void){
   ...
   AntiwindupInit(&reg_rpm, KRW, TIW, w_max, w_min);
...
__interrupt void  adc_isr(void)
{
	...
	if(praca == 0)
	{
	    AntiwindupReset(&reg_iq);
	    AntiwindupReset(&reg_id);
	    AntiwindupReset(&reg_rpm);
	}
    ...
    AntiwindupCalc(&reg_iq, reg_rpm.y_sat, abc2dq.y[1], Ts);
